...
sales_df = pd.read_parquet(
    pathlib.Path(pathlib.Path(__file__).parent.parent, "raw", "retail_sales", "sales")
)
...
store_df = pd.read_parquet(
    pathlib.Path(pathlib.Path(__file__).parent.parent, "raw", "retail_sales", "store")
)
...
sales_df = sales_df[[c for c in sales_df.columns if not c in ['Promo2', 'Promo2SinceWeek',
       'Promo2SinceYear', 'PromoInterval']]]

dd.from_pandas(sales_df, npartitions=6).to_parquet(
    pathlib.Path(pathlib.Path(__file__).parent.parent, "datasets", "retail_sales"),
    schema="infer",
)
