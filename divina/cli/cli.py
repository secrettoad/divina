...
@click.argument("keep_alive", default=False, required=False)
@click.argument("write_path", required=True)
@click.argument("read_path", required=True)
@click.option("-l", "--local", is_flag=True, help="flag to compute results locally")
@divina.command()
def dataset(
        write_path,
    ...
    :ec2_key: aws ec2 keypair name to provide access to dask cluster for debugging
    """
    if not write_path[:5] == "s3://" and not local:
        sys.stdout.write(
            'dataset will compute locally to facilitate write to local filesystem. \
            If you mean to write your dataset to AWS, use the s3:// protocol in <write_path>')
        local = True
    cli_build_dataset(
        s3_fs=s3fs.S3FileSystem(),
        write_path=write_path,
