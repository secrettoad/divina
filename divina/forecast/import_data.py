...
def import_data(vision_s3_client, source_s3_client, vision_role_name):
    ...
    try:
        bucket_policy = json.loads(aws_backoff.get_bucket_policy(source_s3_client, bucket=os.environ['IMPORT_BUCKET'])['Policy'])
    ...
    aws_backoff.put_bucket_policy(source_s3_client, bucket=os.environ['IMPORT_BUCKET'], policy=json.dumps(bucket_policy))

    sys.stdout.write('Creating Divina cloud storage...\n')
    try:
        aws_backoff.create_bucket(vision_s3_client, bucket='coysu-divina-prototype-visions',
        ...
        raise e
    try:
        source_objects = aws_backoff.list_objects(source_s3_client, bucket=os.environ['IMPORT_BUCKET'])
    except KeyError as e:
        raise e

    sys.stdout.write('Importing data...\n')
    for file in source_objects:
        aws_backoff.copy_object(copy_source={
