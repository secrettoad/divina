from divina.ensembles.linear import GLASMA, GLASMAModel
from divina.preprocessing.base import MeanFiller, CategoricalEncoder
...
def test_glasma_output_1(spark_context, test_df_1):
    ...
    df = vector_assembler.transform(df)
    df = model.fit(df).transform(df)
    pd.testing.assert_frame_equal(df.toPandas(), pd.read_pickle('test_glasma_output_1.pickle'))
...
def test_mean_filler_output_1(spark_context, test_df_2):
    ...
    mf = MeanFiller(input_col='a')
    df = mf.transform(df)
    pd.testing.assert_frame_equal(df.toPandas(), pd.read_pickle('test_mean_filler_output_1.pickle'))
...
def test_categorical_encoder_output_1(spark_context, test_df_3):
    ...
    ce = CategoricalEncoder(input_cols=['a', 'b', 'c'])
    df = ce.transform(df)
    pd.testing.assert_frame_equal(df.toPandas(), pd.read_pickle('test_category_encoder_output_1.pickle'))
